...
---
## Le metriche
Le prestazioni del sistema di memoria si misurano con il **miss rate** (o **hit rate**) e con il **tempo medio di accesso alla memoria**.
...
Il tempo di accesso alla cache si paga **sempre**. Il tempo della memoria principale si paga **solo** quando la cache fallisce (probabilità $MR_{cache}$). Il tempo del disco si paga solo quando **entrambe** fallitcono.

La struttura è **ricorsiva**: si può estendere a più livelli di cache (L1, L2, L3) annidando ulteriormente i termini.

## Esempio
Cache con $t_{cache} = 1$ ciclo e $MR = 10\%$; memoria principale con $t_{MM} = 100$ cicli. Ignorando la memoria virtuale:
...
> Con una penalità di miss così alta, **una piccola variazione del miss rate ha un effetto enorme sull'AMAT**. È per questo che si investe tanta complessità ([[Cache - organizzazione e parametri|associatività]], blocchi, politiche di sostituzione) per guadagnare pochi punti percentuali di miss rate.
