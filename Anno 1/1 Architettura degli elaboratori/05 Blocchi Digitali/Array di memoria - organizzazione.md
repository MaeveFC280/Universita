...
I sistemi digitali richiedono **memorie** per conservare grandi quantità di dati. I
registri costruiti con flip-flop sono un tipo di memoria, ma memorizzano piccole
quantità di dati.
...
Gli array di memoria sono costruiti come array di **bit cell**, ognuna delle quali
memorizza **1 bit**.
...
Per ogni combinazione degli ingressi di indirizzo, **un decoder attiva esattamente una
wordline** (→ [[Decoder]]), abilitando la riga corrispondente.
...
## Da ricordare
- $2^N \times M$: $N$ bit di indirizzo, $M$ bit di dato, $2^N$ righe.
- Wordline = riga (selezionata dal decoder); bitline = colonna.
- Lettura: bitline flottante poi pilotata dalla cella. Scrittura: bitline forzata.
- Numero di porte = quanti accessi simultanei.

## Domande flash
1. Quanti bit di indirizzo ha una memoria $1024 \times 32$? Quanti bit contiene?
2. Perché in lettura la bitline si lascia prima flottante?
